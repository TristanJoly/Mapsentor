...
const AplSapaComparison = ({ departments, allData }: { departments: DepartmentData[]; allData: DepartmentData[] }) => {
  ...
  return (
    <div className="p-4 rounded-xl bg-card border border-border shadow-card">
      <h4 className="text-sm font-semibold text-foreground mb-4 flex items-center gap-1">
        APL service d'aide (SAPA)
        <ChartInfoButton title="APL SAPA comparé" text="Nombre d'employés (en Équivalents Temps Plein) pour 100 000 personnes de 60 ans et plus." howToRead="Plus la valeur est élevée, meilleur est l'accès aux services d'aide." source="DREES – Panorama statistique 2024" />
      </h4>
      <ResponsiveContainer width="100%" height={250}>
        <BarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
          <XAxis dataKey="name" tick={{ fontSize: 9 }} />
          <YAxis tick={{ fontSize: 10 }} tickFormatter={(v) => `${v.toFixed(0)} ETP`} />
          <Tooltip formatter={(value: number) => `${value.toFixed(1)} ETP / 100k 60+`} contentStyle={tooltipStyle} />
      ...
      <div className="text-xs text-center text-muted-foreground mt-1">Moyenne nationale : {avgFrance.toFixed(1)} ETP / 100k 60+</div>
    </div>
  );
};
